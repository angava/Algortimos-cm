...
def candidatos(G,N,comp, Grupo):
    ...
    #sacamos un grado promedio entre kcore y dgrade.
    nodosPool = []
    #dKore1 = core_number(G)
    ...
    for i in range(r):
        ...
        tempCandidatos = [j for j in tempCandidatos]
...
def main(argv):
    ...
    prepartition = core_number(G) #se halla el diccionario con los grados k de cada nodo

    tempgrupo = []
    pregrupo = [] #variable que guardar los grupos de nodos por grado
    for i in range(max(list(prepartition.values()))):
        tempgrupo = [j for j in prepartition
            if (i + 1) == prepartition[j]]
