...
def pajek_group(G, Grupo,comp):
    ...
    Grp = [] #guarda los grupos como arreglo para poder que la funcion blockmodel separe los que sobra.
    Grp = [Grupo[i] for i in Grupo]
...
def candidatos(G,N,comp, Grupo):
    ...
    kSeed = []
    while len(kSeed) != parettoNodos and listaInf != []:
           posMax = listaInf.index(max(listaInf))
           vecinos = N.neighbors(listaNodos[posMax])
           controlVecino = 0
           for j in vecinos:
               if j in kSeed:
                  controlVecino = 1
           if controlVecino == 0:
               kSeed.append(listaNodos[posMax])
           listaInf.pop(posMax)
           listaNodos.pop(posMax)
    ...
    for i in Grupo:
         if i in kSeed:
            ...
            dgMax = 0
            for j in Grupo[i]:
                if dgMax < dGcom[j]: dgMax = dGcom[j]
            conteo = 0
            while conteo < parettoGrupo:
                  for j in Grupo[i]:
                      if dGcom[j] == dgMax and not (j in nodosPool) and conteo < parettoGrupo:
                         nodosPool.append(j)
                         conteo = conteo + 1
                  dgMax = dgMax - 1
